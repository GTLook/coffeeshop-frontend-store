...
render() {
  const props = this.props
  const {activeOrder} = props
  if (props.orders.length && !props.activeOrder.id)
    handleOrderSelection(props, props.orders[0])
  return (<div className='container'>
    <section className='order-queue'>
      {
        props.orders.filter(order => order.orderItems.length).sort((a, b) => {
          if (a.is_fulfilled && !b.is_fulfilled)
            return -1
          if (b.is_fulfilled && !a.is_fulfilled)
            return 1
          return Date.parse(a.pickup_time) - Date.parse(b.pickup_time)
        }).map(order => (<div className={`left-card ${order.is_fulfilled
            ? 'fulfilled'
            : null}`} key={order.id} onClick={() => handleOrderSelection(props, order)}>
          ...
          <h3 className='card-order-id'>ORDER ID #{
              order.order_shortid
                ? order.order_shortid.toUpperCase()
                : null
            }</h3>
        </div>))
      }
    ...
    <section className='order-item'>
      <header className='order-header'>
        <div>
          <h1 className='order-id'>ORDER #{
              activeOrder.order_shortid
                ? activeOrder.order_shortid.toUpperCase()
                : null
            }</h1>
          <h1 className='order-customer-name'>{activeOrder.first_name}
            {activeOrder.last_name}</h1>
        </div>
      ...
      </header>
      {
        activeOrder.orderItems
          ? activeOrder.orderItems.map(item => <div className='order-card' key={item.id}>
            <h1>{item.item_name + ' - ' + item.product_size}</h1>
            <ul>
              <li>{item.espresso_shots + item.extra_espresso_shots}
                SHOTS</li>
